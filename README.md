...
## 更新（2026-09-12）：精确表面测地标签 + 解剖式消融 + 三篇 baseline 对比

本次更新加入三块内容，代码均在本仓库中可直接复现。

### 1. 监督标签：从「网格折线距离」换成「真实表面测地距离」

原先的标签是网格图上的 Dijkstra 最短路（只能沿网格边、在顶点拐弯），
相对真实曲面测地距离**平均高估 10.45%**（中位 +7.66%）。

现改用 potpourri3d 的 **flip-out 测地路径算法**（Sharp & Crane 2020）重算标签：
- scripts_local/surface_labels_shard.py —— 分片独立进程生成标签
- scripts_local/surface_poison_fill.py —— 对约 2.8% 会导致求解器段错误的点对做隔离，并用热方法回填
- scripts_local/swap_surface_labels.py —— 把标签替换进采样缓存，原图距离自动备份为 *_graph.csv

EP_high（1,392,236 顶点）结果：15 万对标签，test MRE 由 2.86%（旧标签口径）变为 **3.22%**（真实口径）。

### 2. 解剖式消融（Ablation Study）

四个配置共用同一真值 / 切分 / 测试集 / 100 轮预算：

| 配置 | test MRE | 相对 M0 |
|---|---|---|
| **M0 总体方法**（分区+highway+三段式+欧氏残差） | **3.29%** | 1.00x |
| A1 去掉欧氏残差（保留分区+highway） | 4.40% | 1.34x |
| A2 去掉分区+highway（保留单 GNN+欧氏残差） | 4.18% | 1.27x |
| A3 两者都去掉（单 GNN 直接预测） | 69.09% | 20.97x |
| A4 纯 3D 欧氏直线（无学习） | 19.31% | 5.86x |

结论：两项技术单独去掉只劣化 27%~34%，**同时去掉劣化 21 倍**（强交互作用）——
欧氏残差是「能不能工作」的前提（物理量纲锚点），地形分区+highway 是「能有多准」的增益。
代码：ablation_ep_high/（single_gnn.py 单 GNN 分支 + plot_ablation.py 出图）。

### 3. 与三篇 baseline 的对比（GeGnn / NeuroGF / LiteGE）

独立工程 baseline_ep_high/，在同一份精确标签、同一切分、同一测试集、同样 100 轮下评测：

| 方法 | 输出参数化 | test MRE |
|---|---|---|
| **本项目方法（三段式+欧氏残差）** | 欧氏残差 | **3.22%** |
| LiteGE | 欧氏残差 | 7.64% |
| GeGnn | 欧氏残差 | 8.25% |
| NeuroGF | 欧氏残差 | 19.34% |
| 三篇 | 论文原样输出 | 51.6% / 63.6% / 97.4% |

要点：三篇的原生输出在本任务上全部失效（均差于「直接量直线」19.31%）；
...
| 分支 | 内容 |
|---|---|
| **main（本分支）** | **核心代码 + 误差最小的成功实验**：真实欧氏曲面测地距离标签（flip-out 近似算法）+ 最优配置（残差幅度 ±1.0，test MRE **0.03220**） |
...
- 新保存的三段式和单 GNN 权重包含 `residual_scale`，加载时自动恢复输出幅度。
- 旧权重仍可加载，但无法推断历史幅度；加载前必须按训练配置设置 `EUCLID_RESIDUAL_SCALE`，否则沿用默认 0.5 并提示。
- 网络结构、预测模式和地形参数仍需与训练一致；本次不改变旧 checkpoint 的权重字典格式。
- 外部标签读取统一无向点对并去重；冲突距离、非有限值、非法编号直接报错。重复行被删除后切分可能变化，应重新导出三份点对供所有方法共用。
- 热方法回填保留 `graph_distance`，并新增 `label_method`；历史未知来源标为 `unknown`，回填标为 `heat`。
- 本次为代码和文档修复，历史实验数值未重跑。
