...
│   ├── 训练文档3.md       #   Exp-6：v1.0.0 架构升级 vs Exp-5 单变量消融
│   ├── 待做.md            #   网络架构优化待做清单（A1~A3, B1）
│   ├── CHANGELOG.md       #   版本更新文档
│   └── note.txt           #   零散笔记
├── README.md              # 本文件
└── requirements.txt
...
### Q9. 采样已经足够代表整张图的距离分布，是否就不用考虑过拟合？

不能这么下结论。**采样代表性只压低了跨分布过拟合的风险，没有消除记忆型过拟合、标签噪声吸收
与评估偏差**——过拟合的根本机制取决于「模型容量 vs 有效样本数 / 标签质量 / 评估集特性」，
与采样是否均匀是两回事（例如样本量仍偏少时，模型可能"记住"节点 id 特征而不是学"距离规律"；
监督为折线测地近似 Q3，容量足够时会拟合到这种系统性偏差里）。

诚实结论：**因单图直推式设定 + 均匀采样覆盖，跨分布过拟合风险较低**，但仍通过 dropout、
weight decay、按 val_mae 早停三重机制防范记忆型过拟合。当前实验（Exp-5，depth=3、
lr=0.001+plateau、dropout=0.1、120 epoch）train-val gap 稳定在 [50, 100] 区间、未观察到
val 上翘；但**加大轮数 / 加大模型 / 加深分区**时仍需持续监控 val 曲线与 gap 走向，
不能靠"采样对了就一定没事"的假设。

> 代码证据：正则化开关见 `main.py`（`--dropout_ratio`、`weight_decay=5e-4`、`--early_stop_patience`
> 与 `--lr_scheduler plateau`）；train/val 逐轮日志与 gap 见 `logs/` 归档；实验对照见 `训练文档2.md`。
