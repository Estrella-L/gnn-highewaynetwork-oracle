...
| `main.py` | 训练主循环（损失、早停、保存）+ 导出 test 点对 CSV |
| `infer_distance.py` | 单对顶点推理 |
| `generate_terrain.py` | 生成规则网格地形 `.off`（配套，不参与训练） |
| `baseline.py` | 非学习基线，读 test 点对文件与模型对比（配套） |

> 说明：`generate_terrain.py`（造地形）与 `baseline.py`（基线评估）是**独立、按需运行**的配套脚本，
> 训练入口 `main.py` 既不 import 也不调用它们——下文的"训练流程"仅指 `main.py` 内部的端到端过程。
> 完整的"造地形 → 训练 → 评估"三步关系见 `README.md` 的「工作流总览」。
...
NeurSC 训练流程
│
├─[输入] data/sample_terrain.off  (2-manifold 三角网格: 顶点 x y z + 三角面)
│
...
│     └─ 导出 test 点对 → outputs/results/<run_name>_test_pairs.csv (s,t,true_distance; 供基线对齐)
│
├─⑧ 逐样本构造三路输入  preprocess.build_synthetic_partition_inputs()
│     │   (每个样本各构造一次输入，再按 --batch_size 批量前向)
...
│     ├─ 按 --batch_size 分块: 合并批内子图/高速图 → 一次 forward → loss → backward → Adam.step
│     ├─ 每轮算 train/val 的 mae/rmse/relative_error
│     └─ 按 val_mae 早停 (early_stop_patience), 记录最佳权重
│
└─⑪ 评估与保存  main.py 末尾
      ├─ 用最佳权重在 test 集评估 (mae/rmse/relative_error)
      ├─ outputs/models/<name>.pt    (模型权重)
      ├─ outputs/results/<name>.txt  (指标)
      └─ outputs/params/<name>.txt   (运行参数快照)
```
...
- `main.py` 随后把 test 集导出为 `outputs/results/<run_name>_test_pairs.csv`（`s,t,true_distance`），
  供 `baseline.py` 用 `--test_pairs_file` 在完全相同的 test 点对上评估非学习基线。
...
### 阶段 ⑩：训练循环
- `main.run_distance_epoch(...)`：按 `--batch_size` 把样本分块，每块用 `DistancePredictor.forward_batch`
  把批内多个样本的 Inner 子图与（复制的）高速图合并成一张大图做**一次** `forward → loss → backward → Adam.step`；
  损失由 `build_loss(--loss_type)` 给出（默认 `log_l1`，跨距离尺度归一化）。`--batch_size 1` 等价旧逐样本行为。
- 每轮算 train/val 的 `mae/rmse/relative_error`；按 `val_mae` 早停并记录最佳权重。

### 阶段 ⑪：评估与保存
- 用最佳权重在 test 集评估；保存 `outputs/models/*.pt`、`outputs/results/*.txt`、`outputs/params/*.txt`。
...
```bash
# （可选）生成适配算法的规则网格地形（仅需 numpy，不需要 torch）
python generate_terrain.py --grid 10 20 40

# 训练（输入 .off）；训练后会在 outputs/results/ 生成 <run_name>_test_pairs.csv
python main.py --off_file sample_terrain.off \
  --max_depth 3 --capacity 32 --inner_mode partition \
  --loss_type log_l1 --num_epoch 30 --batch_size 32 --device cuda

# 非学习基线（纯 Python，读上一步导出的 test 点对文件，与模型对比）
python baseline.py --off_file sample_terrain.off \
  --test_pairs_file outputs/results/<run_name>_test_pairs.csv

# 仅派生/查看四叉树分区+高速（纯 Python，不需要 torch）
python build_highway.py --off_file data/sample_terrain.off \
  --max_depth 3 --capacity 32 --out_prefix terrain

# 推理（四叉树参数、inner_mode 必须与训练一致）
python infer_distance.py --model_path outputs/models/<ckpt>.pt \
...
flowchart TD
    ...
    F --> I["build_synthetic_partition_inputs<br/>三路输入(逐样本构造→按batch批量前向)"]
    H --> I
    I --> J["DistancePredictor<br/>InnerGNN×2 + InterGNN + Fusion MLP"]
    J --> K["run_distance_epoch<br/>loss/backward/Adam + 早停"]
    K -->|每轮 val_mae| K
    K --> L["test 评估 + 保存<br/>outputs/models/results/params"]
```
