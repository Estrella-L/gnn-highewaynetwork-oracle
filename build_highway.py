    ...
    for g, dl in iter_source_distances(n_full, eu_g, ev_g, ew_g, boundary_sorted, weighted=weighted):
        ...
        for b in cell_members[leaf_of[g]]:
            ...
            if b != g and db != float("inf"):
                transit_candidates.append((float(db), b))
        if transit_k and transit_k > 0:
            transit_candidates.sort(key=lambda x: x[0])
            transit_candidates = transit_candidates[:transit_k]
        for db, b in transit_candidates:
            _add_edge(g, b, db)
...
def _pipeline_cache_key(off_path, max_depth, capacity, adaptive, feature_dim, transit_k=0):
    """缓存键：.off 文件名 + 内容指纹 + 四叉树参数 + 特征维度 + transit 稀疏化。"""
    base = os.path.splitext(os.path.basename(off_path))[0]
    mode = "ada" if adaptive else "uni"
    fp = _file_fingerprint(off_path)
    tk = int(transit_k or 0)
    return f"{base}_d{max_depth}_c{capacity}_{mode}_f{feature_dim}_tk{tk}_{fp}"
...
def build_graph_and_partition(
    off_path,
    max_depth=3,
    capacity=32,
    adaptive=True,
):
    """轻量入口：只做 .off → 全局图 + 四叉树分区（**不做**高速 Dijkstra 预处理）。

    供"去掉地形分区与 highway 网络"的消融分支使用：该分支只用一张全图上的单个 GNN，
    并不需要 highway 上下文，因此无需付 K 次全图 Dijkstra 的代价。
    仍然返回 leaf_of，以便按同分区/跨分区分组评估，保证与三段式主方法同一口径。

    Returns:
        (graph_info, coords, leaf_of, num_leaves)
    """
    vertices, faces = load_off(off_path)
    graph_info, coords = build_mesh_graph(vertices, faces)
    leaf_of, num_leaves, _occupied = build_quadtree(coords, max_depth, capacity, adaptive)
    return graph_info, coords, leaf_of, num_leaves
...
def build_pipeline_inputs_cached(
    ...
    if cache_dir is None:
        return build_pipeline_inputs(off_path, max_depth, capacity, adaptive, weighted, feature_dim, device, transit_k=transit_k)

    import torch

    os.makedirs(cache_dir, exist_ok=True)
    key = _pipeline_cache_key(off_path, max_depth, capacity, adaptive, feature_dim, transit_k=transit_k)
    cache_pt = os.path.join(cache_dir, key + ".pt")

    if os.path.exists(cache_pt):
        # Cache files are generated by this project and contain Python containers
        # such as defaultdict, which PyTorch 2.6+ blocks under weights_only=True.
    ...
    graph_info, coords, leaf_of, num_leaves, context = build_pipeline_inputs(
        off_path, max_depth, capacity, adaptive, weighted, feature_dim, device="cpu", transit_k=transit_k
    )
    torch.save(
        {
            "graph_info": graph_info,
...
def build_parser():
    ...
    p.add_argument("--transit_k", type=int, default=0, help="keep only nearest k in-cell transit edges per highway node; 0=full")
